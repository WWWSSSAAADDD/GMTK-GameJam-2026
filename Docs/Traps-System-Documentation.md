...
- **MovingPlatformTrap / CageDropTrap:** 检查 Tag 为 `"Player"` 或存在 `CharacterController`
- **MemePusherTrap / DirectionalWallTrap:** 额外检查 `CharacterController` 的 Tag 也需为 `"Player"`
...
- `MemePusherTrap` 和 `DirectionalWallTrap` 使用 `CharacterController.Move()` 推动玩家
